...
export class GameScene extends Phaser.Scene {
  ...
  private createAttackHitbox(direction: number) {
    ...
    // Remove<boltAction type="file" filePath="src/game/scenes/GameScene.ts">    // Remove hitbox after attack animation duration
    ...
    playerStates.forEach(playerState => {
      ...
      if (playerState.x !== undefined && playerState.y !== undefined) {
        ...
        if (this.otherPlayers.has(playerId)) {
          ...
          // Health update logic
          // Check if we have a locally tracked damage value that's more recent
